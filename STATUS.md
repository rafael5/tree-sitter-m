...
| 8 | Editor demonstration (VS Code extension or nvim-treesitter PR) | ⚠️ implementation done; marketplace publish pending. Sibling repo `tree-sitter-m-vscode` (https://github.com/m-dev-tools/tree-sitter-m-vscode) ships a two-layer extension: TextMate grammar for cold-load + a `DocumentSemanticTokensProvider` powered by `tree-sitter-m` compiled to WASM via `tree-sitter build --wasm --docker`. `vsce package` produces a 1.27 MB .vsix bundling the parser .wasm + web-tree-sitter runtime. Marketplace `vsce publish` needs a personal access token from dev.azure.com — gated on user. |
...
6. **B7 — at least one editor integration** (criterion #8). VS Code
   ...
   `github.com/m-dev-tools/tree-sitter-m-vscode` 2026-04-26. Two-layer:
